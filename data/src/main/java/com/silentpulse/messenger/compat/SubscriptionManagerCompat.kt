 ...
 */
package com.silentpulse.messenger.compat
...
class SubscriptionManagerCompat @Inject constructor(context: Context, private val permissions: PermissionManager) {
    ...
    init {
        @Suppress("DEPRECATION") // SubscriptionManager.from() deprecated API 31+; replacement requires API 31
        subscriptionManager = if (Build.VERSION.SDK_INT >= 22) SubscriptionManager.from(context) else null
    }

    @Suppress("DEPRECATION") // addOnSubscriptionsChangedListener overload deprecated API 30+
